...
The Tile List component arranges related tiles in a responsive wrapping layout without adding managed focus or selection behavior.
...
<StoryEmbed
  ...
  caption="Tile List provides responsive wrapping and list semantics for direct `fd-tile` children. Open Storybook to inspect spacing variants and full example content."
/>

### Basic usage

```html
<fd-tile-list label="Benefits links">
  <fd-tile
    id="health-tile"
    tone="neutral"
    icon-name="download"
...
| Name | Description |
|---|---|
| (default) | Author one or more `fd-tile` children. |
