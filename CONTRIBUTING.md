...
Use this section for common contributor paths. For the full workflow, generated-file rules, and validation requirements, follow [.context/AGENT_GUIDE.md](./.context/AGENT_GUIDE.md).

### Add a component

- Run `npm run new:component -- --name component-name --kind first-class|supporting-standalone|supporting-embedded`.
- Implement the component in `packages/components/src/components/`.
- Add or update colocated tests, Storybook stories, and docs guidance.
- Run `npm run sync:components`, then `npm run validate:components`, `npm run test:components`, and `npm run build`.

### Fix a bug

- Change the smallest surface that fixes the behavior.
- Add or update the nearest colocated test first or alongside the fix.
- If the bug affects docs, Storybook behavior, or accessibility guidance, update those artifacts too.
- Run the validators for the touched area, then run `npm run build` before finishing.

### Update docs

- Edit curated docs in `apps/docs/` unless the file is generated by the component sync pipeline.
- Keep examples realistic and plain-language.
- If the change touches public component behavior or metadata, run `npm run validate:components`.

### Update tokens

- Change token sources in `packages/tokens/`.
- Run `npm run validate:tokens`.
- Run `npm run build` if the token change affects downstream package output or docs rendering.

### Add a story

- Add or update the relevant file in `apps/storybook/src/`.
- Include `play` coverage for high-complexity or validation-sensitive behavior.
- Run `npm run validate:components` when public component docs or metadata are involved.
- Run `npm run test:storybook` for meaningful story behavior changes.
